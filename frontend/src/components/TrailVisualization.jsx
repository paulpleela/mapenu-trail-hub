...
import { AlertCircle, Mountain, TreePine, Map, Activity, Eye, Download } from 'lucide-react';
...
const TrailVisualization = ({ trailId, trailName }) => {
  ...
  const fetchAnalysisData = async (analysisType = 'combined') => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/trail/${trailId}/${analysisType}-analysis`);
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="text-2xl font-bold text-green-600">
                {analysis_summary.dem_available ? '✓ Available' : '✗ Not Available'}
              </div>
              {dem_analysis && (
                <div className="text-sm text-gray-600 space-y-1">
                  <p>Elevation Range: {dem_analysis.statistics?.elevation_range?.[0]?.toFixed(1)}m - {dem_analysis.statistics?.elevation_range?.[1]?.toFixed(1)}m</p>
                  <p>Data Points: {dem_analysis.total_points?.toLocaleString()}</p>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="text-2xl font-bold text-blue-600">
                {analysis_summary.lidar_available ? '✓ Available' : '✗ Not Available'}
              </div>
              {lidar_analysis && (
                <div className="text-sm text-gray-600 space-y-1">
                  <p>Point Cloud: {lidar_analysis.total_lidar_points?.toLocaleString()} points</p>
                  <p>Vegetation: {lidar_analysis.statistics?.vegetation_percentage?.toFixed(1)}%</p>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{analysis_summary.dem_available ? 'Yes' : 'No'}</span>
                </div>
                <div className="flex justify-between">
                  <span>LiDAR Coverage:</span>
                  <span className="font-medium">{analysis_summary.lidar_available ? 'Yes' : 'No'}</span>
                </div>
              </div>
            </div>
          </CardContent>
    ...
    if (!analysisData?.dem_analysis) {
      return (
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-medium text-gray-900">No DEM Data Available</h3>
          <p className="text-gray-600">DEM elevation data is not available for this trail location.</p>
        </div>
      );
    }

    const { statistics, elevation_profile, visualization, data_sources } = analysisData.dem_analysis;
  ...
  const LiDARAnalysisTab = () => {
    if (!analysisData?.lidar_analysis) {
      return (
        <div className="text-center py-8">
          <AlertCircle className="w-12 h-12 mx-auto text-gray-400 mb-4" />
          <h3 className="text-lg font-medium text-gray-900">No LiDAR Data Available</h3>
          <p className="text-gray-600">LiDAR point cloud data is not available for this trail location.</p>
        </div>
      );
    }

    const { vegetation_analysis, visualization, data_sources, statistics } = analysisData.lidar_analysis;
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 bg-brown-50 rounded">
                <div className="text-2xl font-bold text-brown-600">
                  {((vegetation_analysis?.ground_points / vegetation_analysis?.total_points) * 100)?.toFixed(1)}%
                </div>
                <div className="text-sm text-brown-800">Ground</div>
                <div className="text-xs text-gray-600">{vegetation_analysis?.ground_points?.toLocaleString()} pts</div>
              </div>
              <div className="text-center p-3 bg-green-100 rounded">
                <div className="text-2xl font-bold text-green-600">
                  {((vegetation_analysis?.low_vegetation / vegetation_analysis?.total_points) * 100)?.toFixed(1)}%
                </div>
                <div className="text-sm text-green-800">Low Vegetation</div>
                <div className="text-xs text-gray-600">{vegetation_analysis?.low_vegetation?.toLocaleString()} pts</div>
              </div>
              <div className="text-center p-3 bg-green-200 rounded">
                <div className="text-2xl font-bold text-green-700">
                  {((vegetation_analysis?.medium_vegetation / vegetation_analysis?.total_points) * 100)?.toFixed(1)}%
                </div>
                <div className="text-sm text-green-900">Medium Vegetation</div>
                <div className="text-xs text-gray-600">{vegetation_analysis?.medium_vegetation?.toLocaleString()} pts</div>
              </div>
              <div className="text-center p-3 bg-green-300 rounded">
                <div className="text-2xl font-bold text-green-800">
                  {((vegetation_analysis?.high_vegetation / vegetation_analysis?.total_points) * 100)?.toFixed(1)}%
                </div>
                <div className="text-sm text-green-900">High Vegetation</div>
                <div className="text-xs text-gray-600">{vegetation_analysis?.high_vegetation?.toLocaleString()} pts</div>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <p className="text-gray-600">
                  This comprehensive analysis combines Digital Elevation Model (DEM) data and Light Detection and Ranging (LiDAR) 
                  point cloud data to provide detailed insights about the trail terrain, vegetation, and surrounding environment.
                </p>
