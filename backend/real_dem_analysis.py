...
class RealDEMAnalyzer:
    ...
    def _coords_to_gda94(self, coords: List[List[float]]) -> List[Tuple[float, float]]:
        ...
        for lat, lon in coords:
            x, y = transformer.transform(lon, lat)  # Note: transformer expects (lon, lat)
        ...
        for dem_file in self.dem_files:
            try:
                with rasterio.open(dem_file) as dataset:
                    ...
                    if (min_x <= bounds.right and max_x >= bounds.left and
                        min_y <= bounds.top and max_y >= bounds.bottom):
    ...
    def extract_elevation_profile(self, trail_coords: List[List[float]]) -> Dict[str, Any]:
                ...
                try:
                    with rasterio.open(dem_file) as dataset:
                        for i, point in enumerate(sample_points):
                            ...
                            if (dataset.bounds.left <= x <= dataset.bounds.right and
                                ...
                                if (0 <= row < dataset.height and 0 <= col < dataset.width):
                                    ...
                                    if elevation != dataset.nodata:
                                        ...
                                        coordinates.append([trail_coords[min(i, len(trail_coords)-1)][0], 
                                                         trail_coords[min(i, len(trail_coords)-1)][1]])
    ...
    def _calculate_trail_bounds(self, gda94_coords: List[List[float]], buffer_meters: int = 0) -> Dict[str, float]:
    ...
    def create_3d_terrain_visualization(self, trail_coords: List[List[float]], buffer_meters: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            with rasterio.open(dem_file) as dataset:
                ...
                min_col, min_row = dataset.index(trail_bounds['min_x'], trail_bounds['max_y'])
                max_col, max_row = dataset.index(trail_bounds['max_x'], trail_bounds['min_y'])
                ...
                windowed_transform = rasterio.windows.transform(window, dataset.transform)
                ...
                try:
                    ...
                    target_points = min(200, total_points)  # Up to 200 points for accuracy
                    sample_interval = max(1, total_points // target_points)
                    sampled_coords = gda94_coords[::sample_interval]
                    
                    print(f"Processing {len(sampled_coords)} trail points from {total_points} total points (interval: {sample_interval})...")
                    ...
                    for i, (x, y) in enumerate(sampled_coords):
                        try:
                            ...
                            if (0 <= windowed_row < elevation_data.shape[0] and 0 <= windowed_col < elevation_data.shape[1]):
                                z = elevation_data[windowed_row, windowed_col]
                                if z != dataset.nodata:
                                    trail_x.append(windowed_col)
                                    trail_y.append(windowed_row)
                                    trail_z.append(z + 1)  # Only 1m above terrain for accuracy
                                    if i < 5:  # Only print first few for debugging
                                        print(f"Added trail point {i}: ({windowed_col}, {windowed_row}, {z})")
                                else:
                                    print(f"Trail point {i}: No data value")
                            else:
                                print(f"Trail point {i}: Out of bounds ({windowed_row}, {windowed_col})")
                        except Exception as e:
                    ...
                    if trail_x:
                        print(f"Creating high-accuracy trail visualization with {len(trail_x)} points")
                        ...
                        print("No trail points found - trail line will not be displayed")
                    ...
                    return self._create_static_3d_plot(elevation_data, gda94_coords, dataset)
                except Exception as e:
    ...
    def _create_static_3d_plot(self, elevation_data, gda94_coords, dataset):
        ...
        try:
            ...
            if trail_x:
                ax.plot(trail_x, trail_y, trail_z, 'r-', linewidth=3, label='Trail Path')
    ...
    def analyze_terrain_features(self, trail_coords: List[List[float]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(1, len(elevations) - 1):
                if elevations[i] > elevations[i-1] and elevations[i] > elevations[i+1]:
                    if elevations[i] - min(elevations[max(0, i-10):i+10]) > 20:  # At least 20m prominence
                        features.append({
                            "type": "Peak",
            ...
            for i in range(1, len(elevations) - 1):
                if elevations[i] < elevations[i-1] and elevations[i] < elevations[i+1]:
                    if max(elevations[max(0, i-10):i+10]) - elevations[i] > 20:  # At least 20m depth
                        features.append({
                            "type": "Valley",
                            "elevation": elevations[i],
                            "distance": distances[i],
            ...
            for i, slope in enumerate(slopes):
                if abs(slope) > 25:  # Steep grade > 25%
                    features.append({
                        ...
                        "slope": slope,
                        "distance": distances[i] if i < len(distances) else distances[-1],
            ...
            return {
                ...
                "summary": {
                    ...
                    "steep_sections": len([f for f in features if "Steep" in f["type"]])
                }
            }
